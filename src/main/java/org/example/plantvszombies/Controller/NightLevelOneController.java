...
import org.example.plantvszombies.Model.Plants.PuffShroom;
import org.example.plantvszombies.Model.Plants.SunFlower;
import org.example.plantvszombies.Model.Plants.SunShroom;
...
public class NightLevelOneController implements Initializable {
    ...
    private enum PlantType { PUFFSHROMM, SUNSHROOM , NONE}
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        plantBar.getChildren().addAll(puffShroomIcon, sunShroomIcon);
        ...
        for (int row = 0; row < GameRoot.getInstance().getROWS(); row++) {
            ...
            Timeline winChecker = new Timeline(new KeyFrame(Duration.seconds(1), e -> {
                if (GameState.getInstance().areAllZombiesSpawned() && GameState.getInstance().getZombies().isEmpty()) {
                    ...
                    if (HelloController.logInPlayer.getLevel()==3) {
